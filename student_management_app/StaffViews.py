...
def staff_home(request):
    ...
    attedance_count = Attendance.objects.filter(subject_id__in=subjects).count()
    ...
    attendance_list = []
    for subject in subjects:
        attedance_count1 = Attendance.objects.filter(subject_id=subject.id).count()
        subject_list.append(subject.subject_name)
        attendance_list.append(attedance_count1)
    ...
    for student in students_attendance:
        attendance_present_count = AttendanceReport.objects.filter(status=True,
                                                                   students_id=student).count()
        attedance_absent_count = AttendanceReport.objects.filter(status=False,
                                                                 student_id=student.id).count()
        student_list.append(student.admin.first_name+" "+ student.admin.last_name)
        student_list_attendance_present.append(attendance_present_count)
        student_list_attendance_absent.append(attedance_absent_count)

    context={
        "Students_count": students_count,
        "attendnace_count": attedance_count,
...
def get_students(request):
    ...
    for student in students:
        data_small={"id":students.admin.id,
...
def get_attendance_dates(request):
    ...
    # Session entroll to Course, Course has Subjects
    # Getting all data from subject based on subject_id
    subject_model = Subjects.objects.get(id=subject_id)
...
def update_attendance_data(request):
    students_ids = request.POST.get("student_ids")
...
def staff_add_result_save(request):
    if request.method != "POST":
        ...
        students_admin_id = request.POST.get('student_list')
        assignment_marks = request.POST.get('assignment_marks')
        exam_marks = request.POST.get('exam_marks')
        subject_id = request.POST.get('subject')

        subject_obj = Students.objects.get(admin=students_admin_id)
        student_obj = Students.objects.get(id=subject_id)
        ...
        try:
            ...
            if check_exist:
                ...
                result = StudentResult(student_id=student_obj,
                                       ...
                                       subject_assignment_mark=assignment_marks)
                result.save()
